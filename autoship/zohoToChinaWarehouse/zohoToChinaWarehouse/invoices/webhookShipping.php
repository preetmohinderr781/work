...
//file_put_contents("ZohoDataWebhookForShipping.txt",$json);

#Get Data from the file
$json1 = file_get_contents('ZohoDataWebhookForShipping.txt');
...
echo $shipping_method_no =$dataTXorder['data']['channel_hawbcode'];


if(empty($dbShippingNumber))
{
    
    $qry="UPDATE `zohoChinaInvoices` SET `ship_no` = '$shipping_method_no' WHERE `order_id` = '$refrence_no'";
  ...
  $curl = curl_init();
                curl_setopt_array($curl, array(
                  ...
                  "data": [
                    {
                      "TX_Bill_Track_No": "'.$shipping_method_no.'"
