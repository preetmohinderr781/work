...
$sql = "SELECT * FROM `warehousee` WHERE date BETWEEN $timeGap AND $timestamp ";
...
foreach ($projects as $row) 
{
    ...
    if ($dbTrackStatus != "delivered" && $dbTrackStatus != "completed") {
        ...
        "includeDetailedScans": true
            ...
            #consignment number same as warehousee
            $FedConsign =
...
$sql = "SELECT * FROM `warehousee` WHERE `ship_no` = '$dbTracking'";
if ($res = mysqli_query($conn, $sql)) {
    if (mysqli_num_rows($res) > 0) {
         while ($row = mysqli_fetch_array($res)) {
             echo '<pre>';
          //print_r($row);
            ...
            #Status from the Database
             $dbStatus=$row['status'];
              ...
              if($dbStatus == "picked")
             {
                file_put_contents('FedOFD2Orders.txt', $LogDate .'--'. $wooId .'--'. $StatusInWords .'--'.$Status.'--'. PHP_EOL, FILE_APPEND);
             }
         ...
         if($dbStatus != 'delivered' && $dbStatus != "completed" )
         {
            ...
            if($dbStatus != $StatusInWords && $StatusInWords != "" )
            
                {
                    ...
                    file_put_contents('fedTest2Orders.txt', $LogDate .'--'. $wooId .'--'. $StatusInWords .'--'.$timestamp. PHP_EOL, FILE_APPEND);
                       # Inserting the data in databse------------
                 $sqlquery = "UPDATE `warehousee` SET `status` = '$StatusInWords' ,`dupStatus` = '1' WHERE `warehousee`.`ship_no` = '$dbOrderShip'";
                if ($conn->query($sqlquery) === TRUE)
                 ...
                 file_put_contents('FedStatus2Orders.txt', $LogDate .'--'. $wooId .'--<b>'. $StatusInWords .'--'.$Status.'--'. PHP_EOL, FILE_APPEND);
