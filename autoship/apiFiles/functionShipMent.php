...
//$sqlquery = "DELETE FROM `wooFunctionShip` WHERE `wooFunctionShip`.`status` = '0'";
//$res = mysqli_query($conn, $sqlquery); 


$sql = "SELECT * FROM `wooFunctionShip`WHERE `status` = '1' LIMIT 1";
        ...
        while ($roww = mysqli_fetch_array($res)) {
            #Array to store the whole values
             ...
             foreach ($projectt as $row)
             {
                      ...
                      {
                          sendDataToWoo($consignment,$wooId);
           
            
            $sqlDBupd = "UPDATE `wooFunctionShip` SET status = '0' WHERE `wooFunctionShip`.`id` = $idd";
