...
require ('../database.php');
 ...
 echo    $LogDate = date("jS  F Y h:i:s A");
echo $timestamp=time();
...
{*/
   file_put_contents('cronworking.txt', $LogDate.'Delivered' . PHP_EOL, FILE_APPEND);
# Checking the Id in database Then the loop will be started 
//$sql = "SELECT * FROM `zohoWarehouse` WHERE `dupStatus` = '1' LIMIT 1";

$sql = "SELECT * FROM `zohoWarehouse` WHERE `dupStatus` = '1'";
...
file_put_contents('res_tracking_log.txt', $LogDate . $OrderStatus.'------'.$dbTrackStatus . $zoho_id. 'Delivered' . PHP_EOL, FILE_APPEND);
...
#send status to the Zoho from the fedex
                $curl = curl_init();
                curl_setopt_array($curl, array(
                  CURLOPT_URL => "https://www.zohoapis.com/crm/v2/Sales_Orders/$zoho_id",
                  CURLOPT_RETURNTRANSFER => true,
...
if ($res = mysqli_query($conn, $sql)) {
       ...
       #Zoho Status from the Database
        $dbStatus=$row['status'];

       #Zoho ID from the Database
...
if ($res = mysqli_query($conn, $sqlquery)) {
   //echo "record inserted successfully";
 }
 } }
